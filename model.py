...
def model(img,detectAngle=False,config={},leftAdjust=False,rightAdjust=False,alph=0.2):
    ...
    newBox1 = sort_box(text_recs)  #按照列高排序,符合我们阅读顺序!     ##下行行文本识别
    result1 = crnnRec(np.array(img),newBox1,leftAdjust,rightAdjust,alph,1.0/f)
    ...
    config['img'] = img
    # 第一种横向书写:
    text_recs, scoreAll2 = text_detect(**config)  ##文字检测
    # 返回的text_recs是一个seq图片
    newBox2 = sort_box(text_recs)  #按照列高排序,符合我们阅读顺序!     ##下行行文本识别
    result2 = crnnRec(np.array(img),newBox2,leftAdjust,rightAdjust,alph,1.0/f)
    scoreAll1=np.array(scoreAll1)
    scoreAll2=np.array(scoreAll2)
    print(scoreAll1,scoreAll2)
    if scoreAll1.sum()>scoreAll2.sum():

        return img, result1, angle
    else:

         return img,result2,angle
