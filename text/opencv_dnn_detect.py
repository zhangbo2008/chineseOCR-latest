...
if opencvFlag=='keras':#keras 文字方向检测,读取的都是同一个.pb权重文件.
    ##转换为tf模型，以便GPU调用
    import tensorflow as tf
    from tensorflow.python.platform import gfile#快速读写模块
    config = tf.ConfigProto(allow_soft_placement=True)
    sess = tf.Session(config=config)
    with gfile.FastGFile(AngleModelPb, 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read()) #这个权重文件里面有图的信息,所以网络信息也不用建立了.
...
def angle_detect_tf(img,adjust=True):
    ...
    index = np.argmax(out,axis=1)[0]#用最大的做旋转
    return ROTATE[index]
...
def angle_detect(img,adjust=True):
    ...
    """
    if opencvFlag=='keras':
        return angle_detect_tf(img,adjust=adjust)#本身keras自己带一个angle算法.
    else:
